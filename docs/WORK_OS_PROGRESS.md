...
**Current checkpoint:** **PHASE 5 — COMPLETE. Phase 6 — IN PROGRESS.** **P6-1 Production Route Containment — COMPLETE / production verified. P6-2 Quality Baseline — COMPLETE. P6-3 Environment + Preview/Staging Safety — IMPLEMENTED / MANUAL VERIFICATION REQUIRED.** P6-4 must not start until the Vercel Production/Preview checks below complete P6-3. Phase 7 remains blocked until every Phase 6 gate passes.
...
- **P6-3 Environment + Preview/Staging Safety — IMPLEMENTED / MANUAL VERIFICATION REQUIRED (2026-08-28).** A framework-independent policy now derives the project reference only from a canonical HTTPS `<project-ref>.supabase.co` URL, requires a publishable browser key, rejects obvious secret/service-role credentials, locks Vercel Production to project `zabpmtkzqetroiwbbofh`, and prevents Vercel Preview from using that project. Vite invokes the policy automatically before an identified Production or Preview build; local/CI builds without `VERCEL_ENV` retain the existing quality workflow. Controlled fixtures prove correct-production and isolated-preview configurations pass while wrong-production, production-bound Preview, and missing Preview configurations fail closed. The shared browser client consumes the same normalized policy result.
  ...
  - **Repository Vercel audit:** `vercel.json` proves Vite builds run through `npm run build`, but repository files cannot reveal Vercel dashboard variable values or their Production/Preview scoping. Before closing P6-3, manually confirm Production has only the production URL and its publishable key, confirm Preview has either a separate non-production URL/key or no backend variables, run one Production deployment, and show that a Preview using production values is rejected before bundling. If an isolated Preview Supabase project is later approved, it will also need its own Edge Function secrets and `APP_URL`; no production Edge Function secret or deployment was changed in P6-3.
...
1. **CURRENT GATE — Phase 6 / P6-3: Environment + Preview/Staging Safety manual verification.** P6-1 and P6-2 are complete; do not start P6-4 or Phase 7 until P6-3 verification passes.
...
## Stop rule
Complete only the current Phase 6 gate. Do not start P6-4 or Phase 7 until P6-3 is verified.
