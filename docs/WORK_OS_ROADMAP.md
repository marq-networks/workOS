...
| 6. Production Foundation | IN PROGRESS — P6-3 MANUAL VERIFICATION | P6-1 Production Route Containment and P6-2 Quality Baseline are complete. P6-3 deployment isolation is implemented and awaits Vercel Production/Preview environment verification. Do not begin P6-4 or mark the phase complete until that verification passes. |
...
2. **Phase 6 — IN PROGRESS:** P6-1 Production Route Containment is complete and production verified; P6-2 Quality Baseline is complete; P6-3 Environment + Preview/Staging Safety is implemented and awaits Vercel dashboard/deployment verification.
...
3. **P6-3 Environment + Preview/Staging Safety — IMPLEMENTED / MANUAL VERIFICATION REQUIRED (2026-08-28):** the Vite build validates canonical HTTPS Supabase URLs and browser-safe keys, locks Vercel Production to project `zabpmtkzqetroiwbbofh`, and rejects missing configuration or the production project in Vercel Preview. Preview may use a valid isolated non-production project when one is approved; otherwise it intentionally fails closed. Repository configuration cannot prove the scopes or current values of variables stored in the Vercel dashboard, so Production and Preview deployment verification remains required before this gate is complete.
