...
| 6. Production Foundation | IN PROGRESS — P6-6 IMPLEMENTED / CI VERIFICATION REQUIRED | P6-1 through P6-5 are complete. P6-6 is implemented locally; authoritative GitHub `Quality` and `browser-smoke` success is still required. |
...
2. **Phase 6 — IN PROGRESS:** P6-1 Production Route Containment, P6-2 Quality Baseline, P6-3 Environment + Preview/Staging Safety, P6-4 Error + Monitoring Foundation, and P6-5 Browser E2E / Critical Auth Smoke Automation are complete. P6-5 authoritative GitHub `browser-smoke` CI passed; P6-6 is implemented and requires authoritative CI verification.
...
6. **P6-6 Initial Bundle + Accessibility Baseline / closeout — IMPLEMENTED / CI VERIFICATION REQUIRED:** a Node-built-in checker measures raw and deterministic level-9 gzip JS/CSS output after the existing production build and enforces 2% ceilings for total/largest JS and total CSS. Chromium accessibility automation covers `/login`, `/work/my-work`, `/org/admin/dashboard`, `/people/members`, and `/super/console` with WCAG 2.0/2.1 A/AA axe tags, critical/serious blocking, moderate/minor reporting, keyboard sanity checks, synthetic Supabase fixtures, and Production-host guards. Local dependency installation was blocked by registry HTTP 403, so authoritative GitHub `Quality` and `browser-smoke` success remains required. Phase 6 stays IN PROGRESS and Phase 7 stays BLOCKED.
