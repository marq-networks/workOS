...
const reopenedTaskStatuses: ReadonlySet<WorkTask['status']> = new Set(['todo','in_progress','blocked']);

function normalizedTaskProgress(current: WorkTask, patch: Parameters<WorkRepository['updateTask']>[1]): number | undefined {
  if (patch.status === 'completed') return 100;
  if (current.status === 'completed' && patch.status && reopenedTaskStatuses.has(patch.status)) return 0;
