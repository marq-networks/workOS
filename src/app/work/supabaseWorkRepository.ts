...
export const supabaseWorkRepository: WorkRepository = {
  ...
  async updateTask(current,patch) { const {assigneeMembershipId,...rest}=patch; const values={...rest,assignee_membership_id:assigneeMembershipId,description:patch.description?.trim(),title:patch.title?.trim(),progress:patch.status==='completed'?100:patch.progress,archived_at:patch.status==='archived'?new Date().toISOString():undefined,updated_at:new Date().toISOString()}; const {data,error}=await supabase.from('tasks').update(values).eq('id',current.id).eq('organization_id',current.organizationId).eq('updated_at',current.updatedAt).select(taskColumns).maybeSingle(); return task(one('work.tasks.update',data as unknown as TaskRow|null,error)); },
};
