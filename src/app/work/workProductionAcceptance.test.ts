...
describe('P7-1 canonical Work integration',()=>{
  ...
  it('keeps progress as a local draft and commits at blur instead of mutating on each keystroke',()=>{expect(screen).toContain('onChange={e=>setDraft(e.target.value)}');expect(screen).toContain('onBlur={()=>void commit()}');expect(screen).not.toContain('onChange={e=>void work.updateTask(t,{progress:');});
